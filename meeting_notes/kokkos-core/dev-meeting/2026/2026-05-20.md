## Wednesday, May 20, 2026 Kokkos Developer Meeting
...
## Notes
- No more 5.1 patch releases
- Status of MDRangePolicy
- Revert if constexpr for static batch size for Cuda causing issues in Trilinos applications
- Performance issue atomics on Cuda (missing Cuda load/store, we were using CAS)
- There are more issues with atomic load for const objects via CAS
- 9122 Events:
  - Carl is concerned about the interface being too low-level
  - Christian says that there are situations where just waiting on execution spaces is too much
  - not an interoperability feature, there is a generic implementation
  - We need a better example justifying the interface
  - better fallback: enqueue a kernel writing some value that can be queried for completion
  - such a feature request has come up a long time ago already
- We want #9185, need another test in a new test file in view subdirectory.
- Need more reviews for #8953
- #7441 no outstanding concerns, we decided to include the OpenACC fix
- Action items from the dev meetings? Luc, Christian, and Damien to discuss

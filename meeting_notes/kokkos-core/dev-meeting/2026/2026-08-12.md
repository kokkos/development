## Wednesday, August 12, 2026 Kokkos Developer Meeting
...
- Merge rights for kokkos/kokkos expanding

- Addressing increasing challenges in leveraging the full bandwidth of modern chips
  - e.g. unrolling, prefetching, simd
  - *we need people to explore and push this*
  - need extensions of existing and potentially new abstractions
  - one example: https://github.com/trilinos/Trilinos/pull/15552
 
- Discuss on whether to add std::execution policy overloads to Kokkos::sort
...
- We add a Intel Arc Pro B70 build in the HPSF CI. Problems with death tests
- 5.2.1:
  - Fix debug options mdspan
  - Fix test in SYCL
  - missing header for debug modules
  - try to create a patch release by the end of the week
- Still waiting for 5.2.0 release deployment in spack, godbolt
- Some more developers have merge rights (Daniel, Nathan, Jakob, Nic, Trevis)
- Addressing increasing challenges in leveraging the full bandwidth of modern chips
  - difficult to do stuff that works across backends
  - might want to add something like `StaticBatchSize` for use inside of kernels
  - Will figure out a group of people to work on it, higher priority than fat binaries and memory spaces
  - Evan will start with working on prefetching
  - Generally working with vendors
- Just-in-time compilation: Interesting but hard to integrate, missing compiler support
- Kokkos::sort with std::execution policy
  - We don't really want to dispatch std::sort with std::execution_policy because that might be messing with other resources
  - We might want to optimize our implementation for host backends
  - Check that std::sort works with Kokkos iterators
  - Generally might want to use the operating system's thread pool for the Threads backend which might provide more compatibility
- Push through Kokkos::single

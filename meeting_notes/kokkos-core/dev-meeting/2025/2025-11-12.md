...
## CI & Nightly Testing
  ...
  Issues:
  - Monthly server maintenance @ SNL this AM -> Luc will regenerate "container device interface" (CDI) files to match updated CUDA drivers
...
- 5.0 Testing:
    - Christian still wants some changes in (8668 are problematic for Roger and Panzer). This was apparently never tested in Sacado, thus it only came up now. **Assignee** Christian
    - Intrepid (fad of fad types) has problems in Debug mode.
    - no other reports reported at CEA/ORNL.
    - Kokkos Kernels is good to go for the release.

 - CI:
    - Daniel was able to reproduce Cuda failures in our CI. We seem to have an empty lib but this can be worked around with setting the cudapath.
    - Luc SNL machines got an update. Drivers would be Cuda 13 ready but these are not the same machines as the ones we are currently using. **Assignee** CI-WG to migrate
 
 - Release:
    - Slides are assigned. When doing the slides also check the documentation **Assignee** https://github.com/kokkos/kokkos-tutorials/pull/144
    - Please update your absence **Assignee** ALL
    - We need to relay the release briefing announcement in the labs. **Assignees** SNL Christian, LANL Patrick, ANL Nevin, CEA Trevis, JSC Prateek, ORNL Jakob
    - Kokkos announcement mailing list seems to not work (Patrick, Jakob, Nic did not get one but linuxfoundation tells we are already on the list)      

 - Discussions:
    - MDRangePolicy: CEA + Nic are meeting tmrw.
    - Precommit ci: Helper for first committers. Python install precommit and it will tell you before you do local commits if they are in a good state. Right now the deployment is still up to discussion. Christian would prefer github action over precommit's runners. We should use it for checking and probably for development
    - 8615: will be merged. **Assignee** Daniel for cherry-picking into 5.0
    - 8612: will be pushed to post 5.0
    - 8668: needs more reviews, see 5.0 testing **Assignee** Nic

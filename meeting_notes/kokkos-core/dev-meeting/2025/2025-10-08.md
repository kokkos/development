...
- **Absences**
  ...
  - Daniel
    - October 14th - October 16th (DARPA)  
  - Jakob
...
- Calendar changed times seems to be resolved
- HPSF: dates confirmed for 16th-20th
  - Kokkos is Thu/Fri, deconflicting with other project meetings
  - team should dip into other meetings and get a feel for what's going on
- Testing
  - Damien wants help triggering SNL CI (especially SNL people!)
  - CI working group is trying to figure out if we can relax the rules
  - Nathan will retrigger some Trilinos checks since things should mostly be working
- 5.0 at the end of October
  - If you want something in 4.7.2, add it to the list
  - Trevis is release manager
    - Dong-hun will provide some help
  - MDRange perforance regression
    - no update from LAMMPS yet
  - View operator() indexing
    - Not reviving the overflow behavior in C++23 version
  - Would be nice to have Jakob's atomics work in there
- Vectorization of range for loops
  - Trott will follow up
- MDRange policy
  - Yuichi will share some notes he took
- Deprecations
  - Damien: would have liked to deprecate the AMD GPU architecture name change (VEGA -> AMD_GFX...)
    - Trott: no sure it's crucial to deprecate this
    - Cedric: need to make sure we don't use it in Spack first
    - Trott: NNSA doesn't use this
  - Damien: removed KEPLER archs since CUDA 12 doesn't support them. Remove Maxwell?
    - Trott, Daniel: no reason to do this. Next big cc step is 7.0. CC7 not supported in CUDA 13, probably we will drop in Kokkos 6.
- JOSS Paper
  - Patrick / Damien will follow up in coming weeks
- Blog Posts:
  - Carl: AMD profiling one under review at SNL, Kokkos Comm to follow
  - Nic: 5.0 release
  - Damien: Kokkos View myths post
- Kernel launch error checking [\#8189](https://github.com/kokkos/kokkos/pull/8189)
  - `cudaGetLastError` after kernel launch, when / where / why?
    - may impact launch latency
    - better stack trace, more meaningful error (not deferred to next fence)
    - do we do this the same way in other backends?
  - `ENABLE_DEBUG_BOUNDS_CHECKS`: enable bounds checks in views
    - apparently we overload this to check launch errors (maybe surprising to people)
  - If you turn bounds checking on, your timing changes so much race conditions will be affected
  - In a normal debug build, you might still get to see race conditions
  - `ENABLE_DEBUG_SYNC`: new additional option to enable debug checks that require device synchronization?
  - We could have a built-in kokkos tool that listens to `KOKKOS_LAUNCH_BLOCKING` environment variable and inserts fences before and after things without having to modify the Kokkos source

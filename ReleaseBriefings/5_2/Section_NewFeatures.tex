...
\begin{frame}[fragile]{SFC64 PRNG Added to Random Algorithms}
  ...
  \begin{itemize}
    \item $2^{64}$ unique streams from single 64-bit seed
    \item Each stream has period length of at least $2^{64}$ (expected $2^{255}$)
    \item Stream offset API enables reproducible distributed initialization
    \item Compatible with existing generators API
  \end{itemize}
